...
/**
 * A customisation of CirclePageIndicator that can take an adapter filled with repeating sets of data,
 * and produce an indicator as if there was only one set of data.  You need to specify how many
 * real items there are in the dataset.
 *
 * @deprecated This is not a great solution to the problem of endless view pagers
 */
public class FakeEndlessCirclePagerIndicator extends View implements PageIndicator {
    ...
    public FakeEndlessCirclePagerIndicator(Context context) {
        this(context, null);
    }

    public FakeEndlessCirclePagerIndicator(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.vpiCirclePageIndicatorStyle);
    }

    public FakeEndlessCirclePagerIndicator(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setCircleSpacing(int spacing) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final int count = (numberOfItems == 0) ? mViewPager.getAdapter().getCount() : numberOfItems;
        ...
        if (mOrientation == HORIZONTAL) {
            longSize = getWidth() + extraSpacing;
        ...
        final float threeRadius = mRadius * 3 + extraSpacing;
        ...
        float cx = (mSnap ? mSnapPage : (numberOfItems == 0) ? mCurrentPage : mCurrentPage %
                numberOfItems) * threeRadius;
//        Log.d(LOGTAG, "currentPage: " + mCurrentPage + " numberOfItems " + numberOfItems + "
// result: " + mCurrentPage % ((numberOfItems == 0) ? 1 : numberOfItems));

        if (numberOfItems != 0 && (mCurrentPage % numberOfItems == numberOfItems - 1)) {
            if (!mSnap && mPageOffset != 0) {
                cx -= mPageOffset * threeRadius * (numberOfItems - 1);
            }
            if (mOrientation == HORIZONTAL) {
                dX = longOffset + cx;
                dY = shortOffset;
